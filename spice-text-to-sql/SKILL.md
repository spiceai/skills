...
description: Generate accurate SQL for Spice.ai's DataFusion engine (PostgreSQL dialect) using your own LLM. Use when writing SQL queries against Spice datasets, converting natural language to SQL, debugging SQL errors, understanding Spice data types, or needing help with Spice-specific functions (ai, embed, vector_search, text_search, rrf, JSON functions). This skill covers schema introspection, type mapping, SQL gotchas, and prompt construction for text-to-SQL workflows.
---
